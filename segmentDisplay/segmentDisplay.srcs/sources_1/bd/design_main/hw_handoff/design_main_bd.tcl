...
proc create_root_design { parentCell } {
  ...
  # Create ports
  set digitSelect [ create_bd_port -dir O -from 3 -to 0 digitSelect ]
  set segmentPins [ create_bd_port -dir O -from 6 -to 0 segmentPins ]
  ...
  set block_name segmentDisplay
  set block_cell_name segmentDisplay_0
  if { [catch {set segmentDisplay_0 [create_bd_cell -type module -reference $block_name $block_cell_name] } errmsg] } {
     catch {common::send_msg_id "BD_TCL-105" "ERROR" "Unable to add referenced block <$block_name>. Please add the files for ${block_name}'s definition into the project."}
     return 1
   } elseif { $segmentDisplay_0 eq "" } {
     catch {common::send_msg_id "BD_TCL-106" "ERROR" "Unable to referenced block <$block_name>. Please add the files for ${block_name}'s definition into the project."}
     return 1
   }
  ...
  # Create instance: uint16_seg_coder_0, and set properties
  set block_name uint16_seg_coder
  set block_cell_name uint16_seg_coder_0
  if { [catch {set uint16_seg_coder_0 [create_bd_cell -type module -reference $block_name $block_cell_name] } errmsg] } {
     catch {common::send_msg_id "BD_TCL-105" "ERROR" "Unable to add referenced block <$block_name>. Please add the files for ${block_name}'s definition into the project."}
     return 1
   } elseif { $uint16_seg_coder_0 eq "" } {
     catch {common::send_msg_id "BD_TCL-106" "ERROR" "Unable to referenced block <$block_name>. Please add the files for ${block_name}'s definition into the project."}
     return 1
   }
